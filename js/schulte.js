  ...
  function mount(host) {
    ...
    function showIntro(msg) {
      ...
      overlay.appendChild(UI.resultPanel({
        ...
        verdict: msg || '按 <b>1 → ' + (L.n * L.n) + '</b> 顺序点击。计时在你点下「开始」的瞬间启动。' +
          '<br>点错会记一次失误并抖动提示，点过的格子<b>不会变色</b>。' +
          '<br><b>计分：</b>用时越短越好；完成 ' + (L.n * L.n) + ' 个数字即算通关，平均每个数字的处理速度决定评级（参考基线 ' + L.target + ' 秒）。' +
          '<br>建议眼睛固定在方格中央，用余光找数字。',
